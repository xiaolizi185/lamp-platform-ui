  ...
  <div>
    <el-form ref="form" :rules="model.rules" :model="model" label-width="100px">
      ...
      <el-form-item label="灯杆类型" prop="fLampPoleTypeId" class="form-item" :rules="model.rules.fLampPoleTypeId">
        <el-select v-model="model.fLampPoleTypeId" placeholder="请选择灯杆类型" style="width: 100%" @change="changeRtuType(model.fLampPoleTypeId)">
          <el-option
    ...
    <el-button style="margin-top: 12px;float: right;margin-right: 5px" type="cancel" @click="reset()">重置</el-button>
    ...
    >
      <map-drawer :drawer="innerMapDrawer" :maplongitude="editparams.fLongitude" :maplatitude="editparams.f_Latitude" :address="editparams.fInstallAddress" @saveMapInfo="saveMapInfo" />
    </el-drawer>
  </div>
</template>
...
export default {
    ...
    return {
      ...
      model: {
        ...
        rules: {
          ...
          fLampPoleTypeId: [
            { required: true, message: '请选择灯杆类型。', trigger: 'change' }
          ],
          fLongitude: [
  ...
  methods: {
    changeRtuType(TypeId){

      this.typeId=TypeId
    },
    init() {
      this.model.fId=this.editparams.fid
      this.model.fProjectId = this.editparams.projectName
      this.model.fLampPoleTypeId = this.editparams.lampTypeName
      this.model.f_Latitude = this.editparams.f_Latitude
      this.model.fLongitude = this.editparams.flongitude
      this.model.fInstallAddress = this.editparams.finstallAddress
      this.model.fRemark = this.editparams.fremark
      this.model.fName = this.editparams.fname
      debugger
      this.model.pId = this.editparams.fprojectId
      this.typeId = this.editparams.flampPoleTypeId
    },
    handleClose(done) {
    ...
    confirmSave() {
     ...
     // this.$refs.form.validate(v => {
       // if (v) {
          const params = {
            'fId': this.model.fId,
            'fProjectId': this.model.pId,
            'fLampPoleTypeId': this.typeId,
          ...
          this.$axios.post('/acloud-lighting/lampPole/update', params).then(res => {
            if (res.data.code === 20000) {
              this.$message.success('修改成功。')
              this.loaddata()
              this.drawer.editInfoDrawer.hide()
            } else {
    ...
    reset() {
      this.model.fProjectId = null
      this.model.fLampPoleTypeId = null
      this.model.f_Latitude = null
      this.model.fLongitude = null
      this.model.fInstallAddress = null
