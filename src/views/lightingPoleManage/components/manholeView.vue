<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column show-overflow-tooltip prop="alias" label="名称" sortable />
      <el-table-column show-overflow-tooltip prop="imei" label="设备标识" sortable />
...
export default {
  ...
  methods: {
    ...
    getTableData() {
      ...
      this.$axios.post('/acloud-lighting/ManholeToLamp/getManholeToLamp?page=' + this.tableData.params.currentPage + '&size=' + this.tableData.params.pageSize, params).then(res => {
    ...
    dealOut(row) {
      if (row.flag === true) {
        this.$axios.post('/acloud-lighting/LampPoleEquipmentInfo/add?lampPoleId=' + this.editparams.fid + '&devId=' + row.id).then(res => {
          if (res.data.code === 20000) {
        ...
        this.$axios.post('/acloud-lighting/LampPoleEquipmentInfo/delete?id=' + row.id).then(res => {
          if (res.data.code === 20000) {
            this.$message.success(res.data.message)
          //  this.getTableData()
          } else {
    ...
    resetQuery() {
      ...
      this.tableData.params.pageSize = 20
      this.getTableData()
    },
    // 修改表格头部颜色
