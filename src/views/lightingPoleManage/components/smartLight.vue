<template>
 ...
 <el-table-column show-overflow-tooltip prop="powerA" label="功率w" sortable />
  <el-table-column show-overflow-tooltip prop="va" label="电压v" sortable />
   <el-table-column show-overflow-tooltip prop="elecs" label="电流a" sortable />
      <el-table-column show-overflow-tooltip prop="createTime" label="创建时间" sortable :formatter="formatCreateDate" />
      <el-table-column show-overflow-tooltip prop="flag" label="分配" width="80">
        <!-- <template slot-scope="scope">
          <el-button v-if="scope.row.flag === false" type="text" style="background-color: #dddddd;color:#fff" @click="dealOut(scope.row)">分配</el-button>
          <el-button v-if="scope.row.flag === true" type="text" style="background-color: #009578;color:#fff" @click="dealOut(scope.row,)">分配</el-button>
        </template> -->
         <template slot-scope="scope">
          <el-switch
...
export default {
  ...
  methods: {
    ...
    getTableData() {
      ...
      this.$axios.post('/acloud-lighting/station/query?page=' + this.tableData.params.currentPage + '&size=' + this.tableData.params.pageSize ,params).then(res => {
    ...
    dealOut(row) {
      if (row.flag === true) {
        this.$axios.post('/acloud-lighting/LampPoleEquipmentInfo/add?lampPoleId=' + this.editparams.fid + '&devId=' + row.autoId).then(res => {
          if (res.data.code === 20000) {
        ...
        this.$axios.post('/acloud-lighting/LampPoleEquipmentInfo/delete?id=' + row.autoId).then(res => {
          if (res.data.code === 20000) {
            this.$message.success(res.data.message)
           // this.getTableData()
          } else {
