<template>
  <div class="app-container">
    <el-row>
      <el-col class="table-content">
        <el-form ref="paramForm" :model="tableData.params" :rules="tableData.paramFormRules" inline label-position="right"
          label-width="80px" class="query-form" style="margin-top: 10px">
          <el-form-item prop="uname" label="设备id" style="font-family: 宋体">
 ...
 <el-row>
   ...
   <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="智慧照明" name="station"></el-tab-pane>
      
       <el-tab-pane label="安防监控" name="monitor"></el-tab-pane>
        <el-tab-pane label="井盖" name="manhole"></el-tab-pane>
         <el-tab-pane label="垃圾桶" name="trash"></el-tab-pane>
    </el-tabs>
    </div>
 </el-row>
    <el-row>
      <el-table v-loading="tableData.listLoading" :header-cell-style="getRowClass" :row-class-name="tableRowClassName"
        :data="tableData.dataList" element-loading-text="Loading" border highlight-current-row style="width: 100%;margin-top: 10px;cursor: pointer"
        :cell-style="changeCellStyle" @selection-change="selsChange">
      ...
      <el-pagination v-show="tableData.total > 0" background :total="tableData.total" :current-page="tableData.params.currentPage"
        :page-sizes="[10,20,30,40,50]" :page-size="tableData.params.pageSize" layout="total,sizes,prev,pager,next"
        style="float: right;margin-top:10px" @size-change="handleSizeChange" @current-change="handleCurrentChange" />
    </el-row>
      ...
      <el-drawer title="发送短信" size="50%" :visible.sync="drawer" :destroy-on-close="true">
        ...
        <el-form ref="form" label-width="120px">
          <el-form-item label="选择运维人员:">
            <el-select ref="sel" v-model="entityObj" placeholder="请选择" @change="changeUser(entityObj)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      for (let i = 0; i < this.selectList.length; i++) {
        if (this.selectList[i].state === 1) {
          ...
          debugger
          checkValue.push(this.selectList[i].stationAlert.autoid)
        }
      }
      ...
      handleClick(tab, event) 
    {
      ...
      else if(tab.name==='monitor')
      {
        this.searchObj.alertDev = 2
           this.getTableData()
      }
       else if(tab.name==='manhole')
      {
      ...
      confirmSave() {
        ...
        var req = {
          stationAlert: this.selectList[0].stationAlert,
          person: this.sendobj,
      ...
      del()
    {
      ...
      for (let i = 0; i < this.selectList.length; i++) {
       {
          delList.push(this.selectList[i].stationAlert.autoid)
       }
      }
      const params = {
            createId: Vue.getAcloudProvider().getUserInfo().id,
