...
export default {
  ...
  methods: {
    init() {
      ...
      this.model.fLatitude = this.editparams.fLatitude
    },
    // 选择经纬度
    selectMap() {
    ...
    confirmSave() {
      ...
      this.$refs.form.validate(v => {
        if (v) {
          this.$axios.post('/acloud-lamppost/project/update', params).then(res => {
            if (res.data.code === 20000) {
              this.$message.success('项目修改成功')
              this.loaddata()
              this.drawer.editProjectDrawer.hide()
            } else {
