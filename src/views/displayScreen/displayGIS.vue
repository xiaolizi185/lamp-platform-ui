<template>
  ...
  <div>
    <div class="mapheader">
      <div>
        <el-row>
          ...
          <span class="choosestation" @click="chooseStation('dark')">选择显示屏:<img src="../../icons/video/display.png" style="margin-left: 10px"></span>
        </el-row>
      </div>
    </div>
    ...
    >
      <displayChild :drawer="stationdrawer1" @getStationInfo="getStationInfo" />
    </el-drawer>
...
<script>
import displayChild from '@/views/displayScreen/displayChild.vue'
import $ from 'jquery'
import Vue from 'vue'
import request from '@/utils/request'
import { getToken, setToken, removeToken } from '@/utils/auth'
import { getAll, openOrclose, brightness } from '@/views/displayScreen/sysLeddevice.js'
Vue.prototype.axios = request
export default {
  components: { displayChild },
  data() {
    return {
      stationdrawer: false,
  ...
  methods: {
    ...
    addMarker() {
      ...
      for (var i = 0; i < that.markerArr.length; i++) {
        var p0 = that.markerArr[i].latitude
        var p1 = that.markerArr[i].longitude
        var point = new BMap.Point(p0, p1)
        var iconImg = that.createIcon('../../icons/video/display.png')
        var marker = new BMap.Marker(point, { icon: iconImg })
        var iw = that.createInfoWindow(i)
        var label = new BMap.Label('', { 'offset': new BMap.Size(that.icon.lb - that.icon.x + 5, -20) })
      ...
      var markerClusterer = new BMapLib.MarkerClusterer(map, { markers: that.markers })
    },
    createwindow(i, marker, label, row) {
      ...
      _marker.addEventListener('infowindowopen', function() {
        ...
        if (document.getElementById('downaction')) {
          ...
          document.getElementById('downaction').addEventListener('click', function() {
            var req = {}
            ...
            } else if (that.actionradio === '3') {
              var req = {
                ledDevice: row,
                bright: that.lampsmask
              }
              brightness(req).then(res => {
                if (res.data.code === 20000) {
                  that.$message.success(res.data.message)
                } else {
                  that.$message.error(res.data.message || '执行失败')
    ...
    createInfoWindow(i) {
      ...
      var iw = new
      BMap.InfoWindow("名称:<b class='iw_poi_title'>" + json.name + '</b>' +
          '<div style="height:150px">' +
          '<div> </hr></div>' +
          '<div style="margin-top:5px">编号：' +
            json.deviceId +
          '</div>' +
          '<div style="margin-top:5px">宽度：' +
          json.width +
          '</div>' +
          '<div style="margin-top:5px">高度：' +
          json.height +
          '</div>' +
          '<div style="margin-top:5px">亮度：' +
          json.light +
          '</div>' +
          '<div style="margin-top:5px">ip地址：' +
          json.ip +
          '</div>' +
          '<div style="margin-top: 5px">动作:<label><input name="radio" type="radio" value="1" />开屏 </label> \n' +
          '<label><input name="radio" type="radio" value="2" />关屏 </label> \n' +
          '<label><input name="radio" type="radio" value="3" />调光 </label> </div>' +
          '<span style="margin-top:15px;display: none;z-index: 102;height: 20px;width: 100%" id="rangelight"  >调光:<input type="range" min="0" max="100" value="50" step="10" id="range" ></span>' +
          '<div> </hr></div>' +
          '</div>' +
          '<div><button style="background-color:#304156;border-color:#304156;color:#fff;cursor:pointer;margin-top: 10px;z-index：100;height: 20px;width: 100%" id="downaction" >执行</button></div>'
      )
      return iw
    },
    ...
    createIcon(url) {
      var icon = new
      BMap.Icon(require('../../icons/video/display.png'),
        new BMap.Size(50, 50),
        {
    ...
    getStationInfo(row) {
      ...
      /*var map = new BMap.Map('XSDFXPage')*/
      // 初始化地图,设置中心点坐标
      var point = new BMap.Point(row.latitude, row.longitude)// 创建点坐标
    ...
    addOneMarker(row) {
      // 获取单灯情况
      var p0 = row.latitude
      var p1 = row.longitude
      var point = new BMap.Point(p0, p1)
      var iconImg = this.createIcon('../../icons/light/lighting-red.png')
      var marker = new BMap.Marker(point, { icon: iconImg })
      var iw = this.createInfoWindow(0)
      var label = new BMap.Label(row.name, { 'offset': new BMap.Size(this.icon.lb - this.icon.x + 5, -20) })
      marker.setLabel(label)
      map.addOverlay(marker)
      label.setStyle({
        borderColor: '#808080',
