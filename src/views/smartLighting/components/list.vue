  ...
  <div class="app-container">
    <el-row>
      <el-col class="table-content">
        ...
        >
          ...
          <el-form-item prop="parentid" label="站点编号" style="font-family: 宋体">
          ...
          <el-form-item>
            ...
            <el-button type="primary" style="background-color: #f0bc34;border-color: #f0bc34" @click="dealStatex()">处理</el-button>
            <el-button type="primary" style="background-color: #53c5a1;border-color: #53c5a1" @click="sendMessage()">发送短信</el-button>
          </el-form-item>
        </el-form>
      </el-col>
    </el-row>
    ...
    <el-row>
      ...
      >
        ...
        <el-table-column show-overflow-tooltip min-width="100" prop="stationAlert.varvalue" label="预警值" />
        ...
        <el-table-column show-overflow-tooltip prop="stationAlert.remarks" label="状态" />
        <el-table-column show-overflow-tooltip prop="stationAlert.state" label="是否处理">
...
export default {
  ...
  methods: {
    ...
    confirmSave() {
      ...
      var req = {
        stationAlert: this.tableChecked[0].stationAlert,
        person: this.sendobj,
    ...
    sendMessage() {
      ...
      this.$axios.post('/acloud-lighting/stationAlert/sendMessage', this.tableChecked).then(res => {
        if (res.data.code === 20000) {
          this.$message.success(res.data.message)
        } else {
          this.$message.error(res.data.message | '发送失败')
    ...
    getPerson() {
      this.$axios.get('/acloud-lighting/person/getAll').then(res => {
        if (res.data.code === 20000) {
          ...
          this.entityObj = this.personList[this.personList.length - 1]
        }
      })
    },
  ...
  formatState(row, column, cellValue) {
    if (cellValue === '1') {
      return '<span style="background-color: #13ce66;color: #fff;">已处理</span>'
    } else {
      return '<span style="background-color: red;color: #fff;">未处理</span>'
    }
  },
