<template>
  <div class="app-container" style="margin-top:10px">
    ...
    <div style="margin-left: 20%">
      ...
      <el-button type="primary" @click="saveEditDatabase">提交</el-button>
      <el-button type="cancel" @click="closeEditDatabase">取消</el-button>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    getDatabaseById() {
      this.$set(this.model, 'name', this.params.name)
      this.$set(this.model, 'description', this.params.description)
      this.$set(this.model, 'url', this.params.url)
      this.$set(this.model, 'userName', this.params.userName)
      this.$set(this.model, 'password', '******')
      // if (this.params.dbType === 'Mysql') {
      //   this.$set(this.model, 'dbType', '1')
      // } else if (this.params.dbType === 'Oracle') {
      //   this.$set(this.model, 'dbType', '2')
      // }
    },
    ...
    saveEditDatabase() {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.$axios.post('/sgid-out-manage/databaseManage/updateDatabase', params).then(res => {
            if (res.data.code === '100001') {
              this.$message.success('编辑成功。')
              this.$emit('confirm', res.data.message)
              this.drawer.editDatabaseDrawer.hide()
              this.reloadTable()
            } else {
              this.$message.error(res.data.message)
            }
